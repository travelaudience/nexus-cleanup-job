...
function load_script {
    ...
    then
        ...
        echo $BODY;
        curl -v -u ${NEXUS_AUTH} -X POST "${NEXUS_URL}/service/rest/v1/script" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"name\": \"dockerCleanup\", \"content\": \"${BODY}\", \"type\": \"groovy\"}"
    fi
}

#  If you want to delete this script, run this function.
function delete_script {
    curl -v -u ${NEXUS_AUTH} -X DELETE "${NEXUS_URL}/service/rest/v1/script/dockerCleanup" -H "accept: application/json"
    if [ $? == 0 ];
    then
...
function run_final_cleanup {
    ...
    do
        ...
        curl -v -u ${NEXUS_AUTH} -X POST "${NEXUS_URL}/service/rest/v1/tasks/$i/run" -H "accept: application/json";
    done
}
...
function create_curl {
    ...
    URL=$(escape_url ${2});
    TIMEFILTER="${3}";
    curl -v -u ${NEXUS_AUTH} -X POST "${NEXUS_URL}/service/rest/v1/script/dockerCleanup/run" -H "accept: application/json" -H "Content-Type: text/plain" -d "{\"repoName\":\"${NEXUS_REPO}\",\"startDate\":\"${DATE}\",\"url\":\"${URL}\",\"timeFilter\":\"${TIMEFILTER}\"}"
}
